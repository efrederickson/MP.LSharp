...
namespace Fireball.Syntax
{
	...
	public class SyntaxDocument : Component, IEnumerable
	{
		...
		private Language mSyntaxData = null;
        ...
        private int UndoRedoNetralStep = 0;
        ...
        /// <summary>
        /// Raised when the syntax data changed
        /// </summary>
        public event EventHandler SyntaxDataChanged;
        ...
        /// <summary>
        /// Raised when a row have been added
        /// </summary>
		...
		public bool Modified
		{
			...
			set
			{
                if (mModified != value)
                {
                    if (!mModified)
                    {
                        // reset undo/redo neutral step
                        UndoRedoNetralStep = UndoStep;
                    }
		...
		/// </summary>
		[DefaultValue(null)]
		public Language SyntaxData
		{
			get { return mSyntaxData; }
			set
			{
				mSyntaxData = value;
                ...
                if (SyntaxDataChanged != null)
                    SyntaxDataChanged(this, EventArgs.Empty);
			}
		}
...
//				Console.WriteLine (this.KeywordQueue.Count.ToString ());
        ...
        /// <summary>
        /// Get the list of all bookmarks for this document
        /// </summary>
        public List<IBookmark> Bookmarks
        {
            get
            {
                ...
                int startIndex = GetNextBookmark(0);
                int nextIndex = startIndex;

                do
                {
                    bookmarks.Add(this[nextIndex].Bookmark);
                    nextIndex = GetNextBookmark(nextIndex);
...
//			}
		...
		public TextPoint Undo()
		{
            ...
            if (UndoStep == UndoRedoNetralStep)
            {
                Modified = false;
            }
		...
		/// <param name="p">Column and Rowindex</param>
		/// <returns>Word object at the given position</returns>
		public Word GetWordFromPos(TextPoint p, bool useLeft)
		{
			Row xtr = this[p.Y];
			int CharNo = 0;
            ...
            for(int i = 0; i< xtr.Count;++i)
			{
                ...
                if ((w.Text.Length + CharNo > p.X && !useLeft) ||
                    (w.Text.Length + CharNo >= p.X && useLeft) ||
                    w == xtr[xtr.Count - 1])
				{
					return w;
				}
...
//			}
        ...
        public Row LineToRow(int line)
        {
            if (line <= mDocument.Count)
            {
                return mDocument[line - 1];
            }
            else
            {
                return null;
		...
		protected internal void OnApplyFormatRanges(Row row)
		{
			...
			else
			{
				...
				foreach (char c in row.Text)
				{
					...
					{
						wn.Style = w.Style;
                        wn.MatchingStyle = w.MatchingStyle;
					}
					else
					{
                        ...
                        wn.MatchingStyle = w.Style;
                        if (w.MatchingStyle == null)
                            w.MatchingStyle = w.Style;
