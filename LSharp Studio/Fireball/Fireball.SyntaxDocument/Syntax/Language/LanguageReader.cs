...
namespace Fireball.Syntax
{
    ...
    public class SyntaxLoader
    {
        ...
        public Language Load(Stream stream)
        {
            ...
            {
                _Language.FileName = (stream as FileStream).Name;
            }
            ...
            if (_UseUserCustomStyles && Directory.Exists(_UserCustomStyles))
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    // Override font values
                    XmlNode userFontNode = userConf.SelectSingleNode("UserConf/Font");
                    XmlNode langFontNode = langDocument.SelectSingleNode("Language/Font");
                    foreach (XmlAttribute userAtt in userFontNode.Attributes)
                    {
                        langFontNode.Attributes[userAtt.LocalName].InnerText = userAtt.InnerText;
                    }

                    // Override text styles values
                    XmlNodeList xlist = langDocument.SelectNodes("Language/Style");

                    foreach (XmlNode current in xlist)
                    {
                        XmlNode userStyleNode = userConf.SelectSingleNode("UserConf/Styles/Style[@Name='" +
                        ...
                        {
                            if (current.Attributes[userAtt.LocalName] == null)
                            {
                                XmlAttribute newAttr = langDocument.CreateAttribute(userAtt.LocalName);
                                newAttr.Value = userAtt.Value;
        ...
        private void ParseLanguage(XmlNode node)
        {
            ...
            _Language.SetStyles(_Styles);
        }

        private void ParseFont(XmlNode node)
        {
            foreach (XmlAttribute att in node.Attributes)
            {
                switch (att.Name.ToLower())
                {
                    case "name":
                        {
                            _Language.FontName = att.Value;
                            break;
                        }
                    case "size":
                        {
                            _Language.FontSize = Int32.Parse(att.Value);
                            break;
                        }
                }
            }
        ...
        private void ParseGutter(XmlNode node)
        {
            GutterInfo gi = new GutterInfo();
            ...
            foreach (XmlAttribute att in node.Attributes)
            {
                switch (att.Name.ToLower())
                {
                    case "style":
                        {
                            gi.Style = GetStyle(att.Value);
                            break;
                        }
                    case "size":
                        {
                            gi.Size = Int32.Parse(att.Value);
                            break;
                        }
                }
            }

            _Language.GutterInfo = gi;
        }
                ...
                if (n.NodeType == XmlNodeType.Element)
                {
                    ...
                    if (n.Name.ToLower() == "bracket")
                    {
                        ...
                        foreach (XmlAttribute att in n.Attributes)
                        {
                            ...
                            if (att.Name.ToLower() == "style")
                                style = att.Value;
                            if (att.Name.ToLower() == "matchingstyle")
                                matchingStyle = att.Value;
                            if (att.Name.ToLower() == "endisseparator")
                                EndIsSeparator = att.Value;
                            if (att.Name.ToLower() == "startisseparator")
                                StartIsSeparator = att.Value;
                        ...
                        if (Start != "")
                        {
                            ...
                            pl.MatchingStyle = GetStyle(matchingStyle);
        ...
        {
            string Name = "";
            string BorderColor = "", ForeColor = "", BackColor = "";
            string DefaultForeColor = "", DefaultBackColor = "";
            string CustomForeColor = "", CustomBackColor = "";
            ...
            foreach (XmlAttribute att in node.Attributes)
            {
                ...
                if (att.Name.ToLower() == "bordercolor")
                    BorderColor = att.Value;

                if (att.Name.ToLower() == "defaultforecolor")
                    DefaultForeColor = att.Value;
                ...
                if (att.Name.ToLower() == "defaultbackcolor")
                    DefaultBackColor = att.Value;

                if (att.Name.ToLower() == "costombackcolor")
                    CustomBackColor = att.Value;

                if (att.Name.ToLower() == "costomforecolor")
                    CustomForeColor = att.Value;
            ...
            if (!string.IsNullOrEmpty(DefaultBackColor))
            {
                st.DefaultBackColor = Color.FromName(DefaultBackColor);
            }

            if (!string.IsNullOrEmpty(CustomBackColor))
            {
                st.CustomBackColor = Color.FromName(CustomBackColor);
            }

            if (!string.IsNullOrEmpty(BorderColor))
            {
                st.BorderColor = Color.FromName(BorderColor);
            }
            ...
            if (!string.IsNullOrEmpty(DefaultForeColor))
            {
                st.DefaultForeColor = Color.FromName(DefaultForeColor);
            }

            if (!string.IsNullOrEmpty(CustomForeColor))
            {
                st.CustomForeColor = Color.FromName(CustomForeColor);
            }
