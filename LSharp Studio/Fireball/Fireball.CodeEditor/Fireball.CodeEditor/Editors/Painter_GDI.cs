...
namespace Fireball.Windows.Forms.CodeEditor.Painter
{
    ...
    public class Painter_GDI : IPainter
    {
        ...
        private void RenderRow(Graphics g, int RowIndex, int RowPos)
        {
            ...
            if (this.Control.ContainsFocus || Control.View.Action == XTextAction.xtDragText || Control.View.Action == XTextAction.xtDragData)
            {
                // It seems like we render the text faster than the IME window is actually rendered!!!!!!!!??????????
                ...
                for (int i = 0; i < wordCollection.Count;i++)
                {
                    ...
                    if (StartDraw)
                    {
                        ...
                        if (w.Type == WordType.xtWord || ws == false)
                        {
                            ...
                            {
                                SetFont(w.MatchingStyle.Bold, w.MatchingStyle.Italic, w.MatchingStyle.Underline, bbuff);
                                bbuff.TextForeColor = w.MatchingStyle.ForeColor;

                                if (w.MatchingStyle.BackColor != Color.Transparent)
                                {
                                    bbuff.TextBackColor = w.MatchingStyle.BackColor;
                                    bbuff.FontTransparent = false;
                                }
                            }
                            ...
                            wdh = bbuff.DrawTabbedString(w.Text, xpos, 0, taborig,
                                                             Control.PixelTabSize).Width;
                            TextStyle bts = Control.BracketMatching && (w == this.BracketEnd || w == this.BracketStart) ? w.MatchingStyle : w.Style;
                            if (bts != null && bts.BorderColor != Color.Transparent)
                            {
                                bbuff.DrawRect(bts.BorderColor, xpos, 0, wdh,
                        ...
                        else if (w.Type == WordType.xtSpace && ws)
                        {
                            ...
                            wdh = bbuff.DrawTabbedString(w.Text, xpos, 0, taborig,
                                                         Control.PixelTabSize).Width;
                            if (w.Style.BorderColor != Color.Transparent)
                            {
                                bbuff.DrawRect(w.Style.BorderColor, xpos, 0, wdh,
                                               Control.View.RowHeight - 1);
                            }
                        }
        ...
        private void RenderCaret(int RowIndex, int ypos)
        {
            ...
            if (Control.View.Action == XTextAction.xtDragText || Control.View.Action == XTextAction.xtDragData)
            {
                //drop Control.Caret
        ...
        private void RenderMargin(int RowIndex)
        {
            ...
            if (Control.ShowGutterMargin)
            {
                ...
                if (RowIndex <= Control.Document.Count - 1)
                {
                    ...
                    if (this.Control.View.RowHeight >= Control._CodeEditor.GutterIcons.ImageSize.Height)
                    {
                        ...
                        if (r.LineMarked)
                            Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, 2);
                        else if (r.WillLineMarked)
                            Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, 3);
                    }
                    else
                    {
                        ...
                        if (r.LineMarked)
                            Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, w, w, 2);
                        else if (r.WillLineMarked)
                            Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, w, w, 3);
                    }
            ...
            if (Control.ShowLineNumbers)
            {
                ...
                if (Control.Document.SyntaxData.GutterInfo != null)
                {
                    bbuff.TextForeColor = Control.Document.SyntaxData.GutterInfo.Style.ForeColor;
                    bbuff.TextBackColor = Control.Document.SyntaxData.GutterInfo.Style.BackColor;
                    SetFont(Control.Document.SyntaxData.GutterInfo.Style.Bold, Control.Document.SyntaxData.GutterInfo.Style.Italic,
                            Control.Document.SyntaxData.GutterInfo.Style.Underline, bbuff);
                }
