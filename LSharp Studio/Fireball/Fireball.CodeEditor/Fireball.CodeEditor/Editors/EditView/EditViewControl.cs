...
//				go.Owner=(Form)this.TopLevelControl;
        ...
        /// <summary>
        /// Select next occurance of the given pattern.
        /// </summary>
        /// <param name="Pattern">Pattern to find</param>
        /// <param name="MatchCase">Case sensitive</param>
        /// <param name="WholeWords">Match whole words only</param>
        /// <param name="UseRegEx">Use of regular expressions</param>
        /// <param name="searchUp">Search from bottom to top</param>
        public bool SelectNext(string Pattern, bool MatchCase, bool WholeWords, bool UseRegEx, bool searchUp)
        {
            FindResult res = this.FindNext(Pattern, MatchCase, WholeWords, UseRegEx, searchUp);
            this.SelectText(res.RowIndex, res.Column, res.Length);
            return res.Matched;
        }
        ...
        /// <param name="UseRegEx">Use of regular expressions</param>
        /// <param name="searchUp">Search from bottom to top</param>
		public FindResult FindNext(string Pattern, bool MatchCase, bool WholeWords, bool UseRegEx, bool searchUp)
		{
            bool passedEOD = false;
			string pattern = Pattern;
            int StartCol = this.Caret.Position.X;
            int StartRow = this.Caret.Position.Y;
            int EndOfSearch = searchUp ? 0 : this.Document.Count;
            int i = this.Caret.Position.Y;

            while (searchUp ? i >= EndOfSearch : i < EndOfSearch)
            {
                Row r = Document[i];
                ...
                if (UseRegEx)
                {
                    ...
                    if (match.Success)
                    {
                        return new FindResult(i, match.Index, match.Length, passedEOD, true);
                    }
                }
                else
                {
                    ...
                    if (((((Col >= StartCol && i == StartRow) || (Col < StartCol && passedEOD) || i != StartRow) && !searchUp) ||
                        (((Col < StartCol - pattern.Length && i == StartRow) || (Col >= StartCol - pattern.Length && passedEOD) || i != StartRow) && searchUp))
                        && Col >= 0)
                    {
                        return new FindResult(i, Col, Pattern.Length, passedEOD, true);
                    }
                }

                if ((!searchUp && i == this.Document.Count - 1) || (searchUp && i == 0))
                {
                    if (passedEOD)
                        break;

                    passedEOD = true;
                    EndOfSearch = StartRow;

                    if (searchUp)
                    {
                        i = this.Document.Count;
                    }
                    else
                    {
                        ++EndOfSearch;
            ...
            return new FindResult(-1, -1, 0, passedEOD, false);
		}
		...
		{
			Document[Caret.Position.Y].Bookmark.Enabled = !Document[Caret.Position.Y].Bookmark.Enabled;
			this.Redraw();
		}
			...
			if (!e.Handled)
			{
				...
				if (!this.ReadOnly)
				{
					switch ((Keys) (int) e.KeyCode)
					{
						...
						case Keys.Tab:
							{
                                if (Selection.IsValid && Selection.Bounds.FirstRow != Selection.Bounds.LastRow)
                                {
                                    if (e.Shift)
                                    {
                                        this.Selection.Outdent();
                                    }
                                    else
                                    {
                                        this.Selection.Indent();
                                    }
                                }
                                else
                                {
            ...
            else
            {
                if (e.X < View.GutterMarginWidth)
                {
                    /*if (_CodeEditor.AllowBreakPoints)
                    {
                        ...
                        r.Breakpoint.Enabled = !r.Breakpoint.Enabled;
                        this.Redraw();
                    }
                    else
                    {
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (Document != null)
            {
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    else if (View.Action == XTextAction.xtDragText || View.Action == XTextAction.xtDragData)
                    {
                    }
                }
                else
		...
		protected override void OnDragOver(DragEventArgs drgevent)
		{
			if (!this.ReadOnly)
			{
				if (Document != null && drgevent.Data.GetDataPresent(typeof(string)))
				{
    ...
    public class FindResult
    {
        ...
        private bool _passedEndOfDoc = false;
        ...
        public FindResult(int rowIndex, int column, int length, bool passedEndOfDoc, bool matched)
        {
            _rowIndex = rowIndex;
            _column = column;
            _length = length;
            _passedEndOfDoc = passedEndOfDoc;
            _matched = matched;
        }
        ...
        public bool PassedEndOfDocument
        {
            get { return _passedEndOfDoc; }
        }
