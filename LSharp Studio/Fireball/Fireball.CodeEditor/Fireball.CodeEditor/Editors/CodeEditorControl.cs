...
namespace Fireball.Windows.Forms
{
	...
	public class CodeEditorControl : Widget
	{
     ...
     //   [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		/// </summary>
		[Category("Appearance - BreakPoints")]
		[Description("The background color to use when there is a breakpoint on the line")]
		...
		{
			_ActiveView.SelectNext(Pattern, MatchCase, WholeWords, UseRegEx, false);
		}
...
//			this.splitView1.Visible = true;
		...
		private void InitializeComponent()
		{
            ...
            this._GutterIcons.Images.SetKeyName(0, "BreakpointEnabled.bmp");
            this._GutterIcons.Images.SetKeyName(1, "BreakpointDisabled.bmp");
            this._GutterIcons.Images.SetKeyName(2, "LineBreakMarker.bmp");
            this._GutterIcons.Images.SetKeyName(3, "WillLineBreakMarker.bmp");
            this._GutterIcons.Images.SetKeyName(4, "BookmarkEnabled.bmp");
            this._GutterIcons.Images.SetKeyName(5, "BookmarkDisabled.bmp");
            // 
            // _AutoListIcons
            // 
        ...
        protected virtual void OnSyntaxDataChanged(object Sender, EventArgs e)
        {
            if (_Document.SyntaxData != null)
            {
                if (!string.IsNullOrEmpty(_Document.SyntaxData.FontName))
                {
                    this.FontName = _Document.SyntaxData.FontName;
                }

                if (_Document.SyntaxData.FontSize > 0)
                {
                    this.FontSize = _Document.SyntaxData.FontSize;
                }

                _Document.SyntaxData.StyleUpdated -= OnSyntaxDataStyleChanged;
                _Document.SyntaxData.StyleUpdated += OnSyntaxDataStyleChanged;
            }
        }

        protected void OnSyntaxDataStyleChanged(object sender, EventArgs e)
        {
            _Document.ReParse();
        }
